...
sidebar_label: systemd-networkd
sidebar_position: 4
---
...
## 6. Consulta e Inspeção (O comando `networkctl`)
...
## 7. Casos Práticos de Configuração (Arquivos `.network`)
...
## 8. Criação de Redes Virtuais (Arquivos `.netdev` + `.network`)
...
## 9. Ciclo de Vida: Aplicando e Testando as Configurações
